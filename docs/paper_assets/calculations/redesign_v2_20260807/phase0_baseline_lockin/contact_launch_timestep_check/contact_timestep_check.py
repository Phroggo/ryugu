#!/usr/bin/env python3
"""Timestep-sensitivity check on a real terrain-contact/launch case, not
just reaction-wheel torque control. The 9.0m commanded hop from this
week's V_GAIN calibration is a confirmed real degraded-mode case
(delivered 0.00894 m/s vs 0.04280 m/s requested, ratio=0.209 -- terrain
drag during the launch stroke scrubbing most of the commanded velocity).
Reruns that exact scenario at the shipped 1ms timestep and a 4x coarser
4ms timestep and compares the delivered velocity / ratio, using the same
stabilization methodology as the original calibration (3 consecutive 2s
samples within 5% magnitude and 0.995 cosine similarity).
"""
...
BRIDGE_YAML = "/tmp/ryugu_bridge_scout_1_ctc.yaml"
DISTANCE = 9.0
G = 1.14e-4
SIN2TH = 0.56
...
class Calib(Node):
    def __init__(self):
        super().__init__('contact_timestep_calib')
        self.landed = None
        self.speed = None
...
def run_one(label, world_file):
    ...
    while time.time() - t0 < READY_TIMEOUT:
        ...
        if node.landed is True and node.speed is not None and node.speed < 0.02:
            break
    log(f"ready check done: landed={node.landed}, speed={node.speed}")

    log(f"commanding jump distance={DISTANCE}m (v_req={V_REQ:.5f} m/s)")
    ...
    if not node.separated:
        ...
        result = {"label": label, "world_file": world_file, "status": "no_separation"}
        node.destroy_node()
        rclpy.shutdown()
        return result
    ...
    if stabilized:
        log(f"STABILIZED: delivered={delivered:.5f} ratio={delivered/V_REQ:.3f} t={stabilize_time:.1f}s")
        result = {"label": label, "world_file": world_file, "distance": DISTANCE,
                  "v_req": V_REQ, "delivered": delivered, "ratio": delivered / V_REQ,
                  "status": "stabilized", "stabilize_time_s": stabilize_time,
                  "n_samples": len(samples)}
    else:
        log("never stabilized within window")
        result = {"label": label, "world_file": world_file, "distance": DISTANCE,
                  "v_req": V_REQ, "status": "never_stabilized", "n_samples": len(samples)}
...
def main():
    ...
    with open(f"{LOG_DIR}/contact_timestep_results.json", 'w') as f:
