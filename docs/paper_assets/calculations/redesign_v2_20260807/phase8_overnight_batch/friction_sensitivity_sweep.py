#!/usr/bin/env python3
"""Phase 8, Priority 3a: foot-friction (mu) sensitivity sweep. mu in
{0.40, 0.50, 0.62 (live baseline), 0.75, 0.90}, n=20 launches at 9.0m
each (100 runs total), same methodology as
phase7_full_revalidation/launch_delivery_batch_n30.py. Uses the model
variants generated by generate_mu_variants.py (spawned via
model://spacehopper_muXXX; mu=0.62 spawns the unmodified, live
model://spacehopper).

Run: python3 friction_sensitivity_sweep.py
"""
import json, math, os, subprocess, time

# Variant model packages (spacehopper_muXXX) live under this experiment's
# own variant_models/ dir, not the real models/ tree (see
# generate_mu_variants.py) -- both paths needed on GZ_SIM_RESOURCE_PATH,
# the real one for mu=0.62 (unmodified model://spacehopper) plus shared
# assets like the skydome, the variant one for the other 4 mu values.
LOG_DIR = os.path.dirname(__file__)
os.environ['GZ_SIM_RESOURCE_PATH'] = (
    '/home/melvin/ryugu_v2_ws/src/ryugu_sim/models:' + f'{LOG_DIR}/variant_models')
BRIDGE_YAML = "/tmp/ryugu_bridge_scout_1_p8mu.yaml"
...
SAMPLE_PERIOD = 2.0
OUT = f"{LOG_DIR}/friction_sweep_results.json"
...
class Calib(Node):
    def __init__(self):
        super().__init__('phase8_mu_sweep')
        self.landed = None
        self.speed = None
...
def start_world(log):
    ...
    gz_log = open(f"{LOG_DIR}/gz_mu_sweep.log", 'a')
...
def spawn_and_launch_nodes(model_uri, label, rep):
    ...
    for name, cmd in [
        ...
        logf = open(f"{LOG_DIR}/{name}_mu{label}_rep{rep}.log", 'w')
