...
Checked this across all 80 trials: **every single trial that reached an outcome (78/80; 2 hit `no_landing`/timeout) shows the give-up message** — not one trial in this batch converged within its own active-attempt sequence. (Correction, 2026-08-13: an earlier version of this report said 76/80 with 4 no_landing; the actual count is 78/80 with 2 no_landing — verified independently against the raw JSON.) Cross-checked against Phase 7's original baseline logs for comparison: **20/20 side_rest and 20/20 full_inversion baseline trials show the identical give-up pattern.** This is consistent, pre-existing system behavior, not something introduced by this phase's harness or the I_bot perturbation — so the recovery-**rate** comparison between baseline and perturbed configs remains valid (both measure the same thing: "did the give-up-then-settle process end up upright within the window," consistently defined).
...
| Bucket | Config | Recovered | Rate | 95% Wilson CI | two-proportion z-test | Fisher's exact test |
|---|---|---|---|---|---|---|
| side_rest | baseline (Phase 7, reused) | 10/20 | 50.0% | [29.9%, 70.1%] | — | — |
| side_rest | ibot_p20 (+20%) | 5/20 | 25.0% | [11.2%, 46.9%] | z=1.63, p=0.102 | p=0.191 |
| side_rest | ibot_m20 (-20%) | 5/20 | 25.0% | [11.2%, 46.9%] | z=1.63, p=0.102 | p=0.191 |
| full_inversion | baseline (Phase 7, reused) | 3/20 | 15.0% | [5.2%, 36.0%] | — | — |
| full_inversion | ibot_p20 (+20%) | 9/20 | 45.0% | [25.8%, 65.8%] | z=-2.07, p=0.038 | p=0.082 |
| full_inversion | ibot_m20 (-20%) | 7/20 | 35.0% | [18.1%, 56.7%] | z=-1.46, p=0.144 | p=0.273 |

With 4 comparisons run, a Bonferroni-corrected significance threshold is p<0.0125 — **none of the four comparisons reach significance at that threshold under either test**. Correction, 2026-08-13: the two-proportion z-test is a normal approximation that gets shaky at n=20 with counts as low as 3 (full_inversion's baseline). Fisher's exact test is the more defensible choice at this sample size and is reported alongside; it gives full_inversion+20% p=0.082 (vs. the z-test's p=0.038) — still short of even the uncorrected 0.05 threshold, so if anything the null-result conclusion in §5 is *more* solidly supported under the more appropriate test, not less. All CIs substantially overlap their respective baselines under both tests.
...
**No, not at the level of statistical significance achievable with n=20/config, but the point estimates move enough to warrant honest reporting rather than a clean "robust" claim.** Side_rest's recovery rate moves from 50% to 25% at *both* +20% and -20% perturbations — a symmetric-looking drop that is not statistically distinguishable from baseline at this sample size (p=0.10 z-test / p=0.19 Fisher's exact), but also not a reassuring "stayed flat at 50%" result; it is more accurately described as "recovery remains partial and unreliable under perturbation, with a point estimate that dropped by half but with wide, overlapping confidence intervals." Full_inversion's rate *increases* under both perturbations (15%→45%/35%); the larger shift is not significant under the more appropriate Fisher's exact test (p=0.082) either, though the point-estimate shift is large enough that "recovery is rare" moving toward "recovery happens roughly a third to nearly half the time" is worth describing qualitatively even though it isn't statistically supported at n=20. Recommend describing this in the paper as: the lumped-mass approximation's effect on self-righting recovery rate could not be ruled out at n=20/config — the qualitative *ordering* of buckets (full_inversion hardest, side_rest intermediate) does not flip, but point estimates shift by up to 2x in both directions, and a larger sample (the same n≥50/bucket scale already used for Phase 10's sensor-noise study) would be needed to state robustness with confidence rather than assert it from an underpowered comparison.
