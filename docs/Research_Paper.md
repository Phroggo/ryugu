...
**Launch stroke geometry.** Total foot–regolith friction capacity is $\mu m g \approx 2.9\times10^{-4}$ N, using $\mu \approx 1.0$ — the simulation's unconfigured default surface-friction coefficient (Gazebo/DART, no explicit `<friction>` block was set on the foot collision geometry), not a value independently derived or measured from real regolith mechanics. This is stated plainly as a modeling limitation rather than left implicit: real Ryugu regolith friction coefficients reported in the planetary-science literature vary with grain size, packing, and cohesion, and $\mu \approx 1.0$ was never checked against that literature — it is simply what the physics engine assumes when no surface friction is explicitly configured. Any lateral component of leg force slides the feet rather than lifting the body regardless of the exact $\mu$ value; the deployed stroke keeps each foot directly beneath its hip through the entire extension (a "zigzag" leg posture — calf angled back inward), so the ground reaction remains vertical independent of this uncertainty.
...
$V_{GAIN}$ itself carries an unresolved calibration gap worth stating directly rather than presenting as settled: it was fitted once, from an early stroke iteration, and was never re-fitted after two subsequent design changes that materially affect delivered velocity — the lean magnitude ($0.5 \to 0.3$ rad, §"Directional hopping" below) and the 90%-release cutoff just described. A direct measurement taken for this paper (§7, "Jump height") shows the consequence: actual delivered launch velocity for a commanded 9 m hop was only $\approx$34% of $v_{req}$, and the actual achieved elevation angle ($\approx$47°, back-derived from the measured apex and range together) differs substantially from the $\approx$73° the geometric model assumes from the lean angle alone. Both numbers indicate $V_{GAIN}$ (and the lean-to-elevation geometric assumption it's paired with) are due for a systematic re-calibration sweep against the current stroke design — flagged here as a known, quantified gap rather than corrected by guessing a new constant without that sweep.
...
* **Jump height:** peak apex altitude of 0.49 m above resting ground level, measured directly from odometry over a complete separation-to-landing cycle for a commanded 9 m hop (resting baseline 4.800 m, apex 5.291 m, horizontal range 1.83 m, all from live telemetry). Combining the measured apex *and* the measured range (two independent readings of the same flight) lets both the actual delivered velocity and the actual achieved launch elevation be solved for directly, rather than assumed: $v_{actual} \approx 0.0145$ m/s ($\approx$34% of the $v_{req} = 0.0428$ m/s commanded) at an elevation of $\approx$47°, against the $\approx$73° the crouch-lean geometric model (§3.1) assumes. Both figures are precisely quantified consequences of the $V_{GAIN}$ calibration gap identified in §3.1 — not a vague appeal to "contact dynamics," but the specific, measured size of the shortfall.
...
**Launch delivery efficiency, checked against a matching-scale, matching-gravity-regime flywheel hopper.** A closer comparison than any above exists for the specific §3.1 finding that only $\approx$34% of commanded launch velocity was delivered: Hockman, Frick, Nesnas, and Pavone's internally-actuated hopper prototype [32] is close to this platform's own scale in every dimension that matters — 2.3 kg mass against this platform's 2.5 kg, 0.013 kg$\cdot$m$^2$ body inertia against this platform's 0.012–0.020 kg$\cdot$m$^2$ range, and flywheels sized at roughly 1/10th total mass, operated at an emulated 0.001 g — genuinely comparable to Ryugu's regime, not a different-planet analogy. Critically, they report a real commanded-vs-achieved hop-distance measurement: a 1 m commanded hop (flywheel driven to a calculated 550 RPM) produced a measured mean distance of 0.94 m across 30 trials ($\sigma = 0.07$ m) — a $\approx$94% delivery efficiency, dramatically higher than this platform's measured $\approx$34%. This comparison is not flattering, and is reported as such: it indicates the gap identified in §3.1 is not simply an unavoidable feature of flywheel/leg-driven micro-gravity hopping in general — a closely comparable system achieves far tighter commanded-to-delivered tracking — and reinforces, with an external benchmark, that the $V_{GAIN}$ re-calibration flagged in §3.1 is a real, addressable deficiency rather than a fundamental limit of the actuation class.
