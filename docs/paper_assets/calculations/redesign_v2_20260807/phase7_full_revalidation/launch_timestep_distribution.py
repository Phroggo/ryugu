#!/usr/bin/env python3
"""Phase 7: launch/contact timestep-sensitivity DISTRIBUTION, rerun of
phase0_baseline_lockin/contact_launch_timestep_check/contact_timestep_distribution.py's
exact methodology (5 repeats per timestep, same 9.0m degraded-mode
scenario, same stabilization definition) but against the FINAL model and
controller stack: current mass model (Phase 2+), current gains (Phase 3),
the genuine-separation-confirmation launch state machine and recalibrated
V_GAIN (Phase 6), and the full 4-node stack (bridge, hopper_locomotion,
attitude_controller, landing_controller) rather than landing_controller
alone. Phase 0's run predates ALL of that (pre-mass-redesign controller
state, tag pre-mass-redesign) -- this is not a re-confirmation of the same
numbers, it's a fresh measurement against the shipped system.

Reuses phase4_attitude_revalidation/ryugu_4ms.sdf for the 4ms world
(confirmed identical to the current worlds/ryugu.sdf except max_step_size).

Run: python3 launch_timestep_distribution.py
"""
import json, math, os, subprocess, time

os.environ['GZ_SIM_RESOURCE_PATH'] = '/home/melvin/ryugu_v2_ws/src/ryugu_sim/models'
LOG_DIR = os.path.dirname(__file__)
BRIDGE_YAML = "/tmp/ryugu_bridge_scout_1_p7ltd.yaml"
...
SAMPLE_PERIOD = 2.0
N_REPEATS = 5
OUT = f"{LOG_DIR}/launch_timestep_distribution_results.json"
...
class Calib(Node):
    def __init__(self):
        super().__init__('p7_launch_timestep_dist')
        self.landed = None
        self.speed = None
...
def start_world(world_file, label):
    gz_log = open(f"{LOG_DIR}/gz_ltd_{label}.log", 'a')
...
def spawn_and_launch_nodes(label, rep):
    ...
    # BUG FIX (found live in launch_delivery_batch_n30.py, Phase 7): must
    # also kill bridge_scout_1 here, or bridges accumulate across repeats
    # within a timestep block and eventually hang the batch (confirmed:
    # 5 stacked bridge_scout_1 processes hung a run for 4+ hours).
    subprocess.run(['pkill', '-9', '-f',
                     'bridge_scout_1|loco_scout_1|attitude_scout_1|landing_scout_1'],
                    capture_output=True)
    time.sleep(1)
    ...
    for name, cmd in [
        ...
        logf = open(f"{LOG_DIR}/{name}_ltd_{label}_rep{rep}.log", 'w')
    ...
    for label, _ in WORLDS:
        ...
        if vals:
            ...
            log(f"{label}: n={len(vals)} stabilized (of {N_REPEATS}, {n_no_sep} no_separation) "
                f"ratios={[round(v,3) for v in vals]} mean={mean:.3f} range={spread:.3f} "
