#!/usr/bin/env python3
"""Phase 6 checkpoint: targeted batch (n=10) at the SAME 9.0m degraded-mode
scenario used by the old distribution check
(phase0_baseline_lockin/contact_launch_timestep_check/, 1ms timestep,
shipped world) -- but now with (a) the genuine-separation-confirmation
launch state machine and (b) the Phase-6-recalibrated V_GAIN, both already
live in hopper_locomotion.py by the time this is run.

Old baseline (pre-Phase-6, pre-mass-redesign controller state, 1ms only):
  n=5, 4/5 stabilized (1 timeout = 20% non-separation for 1ms alone;
  combined with the 4ms leg, 3/10 = 30% never separated at all), ratios
  0.009, 0.301, 0.013, 0.210 -> range 0.009-0.301 (union with 4ms:
  0.009-0.447).

Checkpoint: non-separation rate improved, ratio spread tighter than the
old 0.009-0.447 range. Same stabilization methodology as every other
launch-velocity measurement in this project (3 consecutive 2s samples
within 5% magnitude and 0.995 cosine similarity).

Run: python3 launch_reliability_batch.py
"""
import json, math, os, subprocess, time

os.environ['GZ_SIM_RESOURCE_PATH'] = '/home/melvin/ryugu_v2_ws/src/ryugu_sim/models'
LOG_DIR = os.path.dirname(__file__)
BRIDGE_YAML = "/tmp/ryugu_bridge_scout_1_batch6.yaml"
...
SAMPLE_PERIOD = 2.0
OUT = f"{LOG_DIR}/launch_reliability_results.json"
...
def start_world():
    gz_log = open(f"{LOG_DIR}/gz_batch6.log", 'a')
...
def spawn_and_launch_nodes(rep):
    ...
    for name, cmd in [
        ...
        logf = open(f"{LOG_DIR}/{name}_batch_rep{rep}.log", 'w')
