...
**Rate-modulated launch.** Early builds scaled hop distance by commanding a *fraction* of the full stroke, assuming separation velocity scales with amplitude. Telemetry falsified this: the position-PID joints stay torque-saturated through most of any stepped stroke, so the legs race at near-terminal speed *regardless* of commanded amplitude — a 27% stroke "9 m" hop was measured separating at 0.19 m/s and flying more than 76 m. The deployed launch instead always uses the full stroke geometry and modulates the stroke *rate*: joint targets are interpolated from crouch to extension over a ramp of duration $T = k/v_{req}$ (with $k$ an empirically calibrated release-window gain), which the joints track without saturating. Two refinements proved essential. First, the interpolation is quadratic (ease-in), because near full extension the leg approaches its straight-leg singularity — $\partial h/\partial\theta \to 0$ — and a linear angle ramp therefore front-loads all body rise and releases the body at a crawl; the eased ramp places peak extension rate at the moment of release. Second, release occurs at 90% amplitude, before the singular final degrees of travel.
