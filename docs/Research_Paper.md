...
* **Self-righting:** recovery from forced full inversion in ~9 s via the reaction-wheel roll, including self-correction of an initial wrong-direction guess. Partial-tilt recovery (the more common case) went through its own fix during this paper's calibration investigation: instrumented telemetry showed the gentle-roll maneuver's single, fixed roll-axis-per-attempt (held for the full 15 s attempt window) reliably rolling the body *toward* upright but overshooting past its peak before crossing the success threshold, then continuing to roll through and back down for the remainder of the attempt — a repeatable failure shape (e.g. one traced attempt: $u_z$ climbing 0.67 $\to$ 0.84, then falling back to 0.22, never reaching the 0.90 threshold) responsible for the majority of observed righting failures and the multi-minute-to-hour uncommanded re-flight cascades they triggered on landing. Re-deriving the roll axis periodically (every $\approx$1 s, rather than once per 15 s attempt or continuously — continuous re-aiming was tried earlier and independently found to chatter destructively near the vertical crossing) resolved this in every post-fix trial observed, including one case that needed 4 of the 5 available attempts.
