#!/usr/bin/env python3
"""Phase 8, Priority 2 (overnight batch): launch delivery, n=100 at the
9.0m degraded-mode scenario, requested to give the reviewers tighter
confidence intervals than Phase 7's n=30. Direct extension of
phase7_full_revalidation/launch_delivery_batch_n30.py -- same methodology,
same STABILIZE_WINDOW=75s and periodic-full-daemon-restart fixes (both
verified clean in Phase 7's rerun after the bridge-leak bug was fixed),
just N_REPEATS bumped to 100 and a 95% CI added to the summary.

Run: python3 launch_delivery_batch_n100.py
"""
import json, math, os, subprocess, time

os.environ['GZ_SIM_RESOURCE_PATH'] = '/home/melvin/ryugu_v2_ws/src/ryugu_sim/models'
LOG_DIR = os.path.dirname(__file__)
BRIDGE_YAML = "/tmp/ryugu_bridge_scout_1_p8n100.yaml"
...
SAMPLE_PERIOD = 2.0
OUT = f"{LOG_DIR}/launch_delivery_n100_results.json"
...
class Calib(Node):
    def __init__(self):
        super().__init__('phase8_launch_n100')
        self.landed = None
        self.speed = None
...
def start_world(log):
    ...
    gz_log = open(f"{LOG_DIR}/gz_batch8_n100.log", 'a')
...
def spawn_and_launch_nodes(rep):
    ...
    for name, cmd in [
        ...
        logf = open(f"{LOG_DIR}/{name}_n100_rep{rep}.log", 'w')
...
def main():
    ...
    if ratios:
        ...
        std_r = math.sqrt(var)
        # Normal-approximation 95% CI on the mean. n is large enough here
        # (>=~30 stabilized expected) for this to be a reasonable choice;
        # flagged in the report as an approximation, not a bootstrap/exact
        # interval.
    ...
    log("--- scanning loco_scout_1_n100_rep*.log for TICK/WALL-TIME MISMATCH markers ---")
    found_any = False
    for rep in range(1, N_REPEATS + 1):
        p = f"{LOG_DIR}/loco_scout_1_n100_rep{rep}.log"
        if os.path.exists(p):
            with open(p) as f:
                for line in f:
